...
class ForwardModel(Model):
    def load_data(self):
        ...
        self.X_train, self.y_train = embedding.generate_embeddings(generate_data.main("data"), \
                embedding_dict, self.config.embed_size, self.config.threshold)
        self.X_dev, self.y_dev = embedding.generate_embeddings(generate_data.main("data"), \
                embedding_dict, self.config.embed_size, self.config.threshold)
        self.X_test, self.y_test = embedding.generate_embeddings(generate_data.main("test"), \
                embedding_dict, self.config.embed_size, self.config.threshold)
...
def run_model(hl, lr):
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session() as session:
            ...
            save_predictions(predictions, "feedforward.predicted")
